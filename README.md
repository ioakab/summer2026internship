...
1. **File discovery** — lists every `.hipo` file in the input folder.
2. **File loop** — for each file:
    - Opens it with `HipoReader`.
    ...
    - **Event loop** — for each event:
        ...
        10. Combines π⁺, π⁻, and the π⁰ candidate into an **ω candidate**, fills `hOmega` with the invariant mass, and fills the ω's own p/θ/φ (from the summed 4-vector) plus a proxy vz (from the π⁺ track — see note in `Histograms.java`).
    - Closes the reader.
3. **Cutflow summary** — prints how many events survived each stage (total read → has electron → topology → vz → DIS → angle → π⁰ window → filled into `hOmega`). **This is the first thing to check if histograms come out empty** — it tells you exactly which cut is killing your statistics.
4. **π⁰ mass fit** — fits a Gaussian to `hPi0` between 0.10–0.17 GeV and prints the fitted mean/sigma.
5. **Plotting** — calls `Plotting.drawAll(h)` to open all canvases.
...
- **ω vertex is a proxy.** The ω is a reconstructed composite (sum of π⁺, π⁻, π⁰ 4-vectors) with no directly measured vertex, so `hOmegaVz` uses the π⁺ track's vz as a stand-in under the assumption that the charged tracks share a common production vertex.
